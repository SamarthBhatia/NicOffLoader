...
RunSummary run_simulation(const CliOptions& options) {
    ...
    const RunMetrics run_metrics = compute_run_metrics(scheduler);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "Completed " << summary.completed_tasks << " tasks in "
                  << format_double(summary.makespan_us, 3) << " us\n";
        std::cout << "Throughput: " << format_double(summary.throughput_per_sec, 3) << " tasks/s\n";
        ...
        if (summary.metrics.aggregate.latency_stats.count > 0) {
            ...
            std::cout << "Latency mean/p95/p99 (us): "
                      << format_double(stats.mean, 3) << " / "
                      << format_double(stats.p95, 3) << " / "
                      << format_double(stats.p99, 3) << "\n";
        }
        return 0;
