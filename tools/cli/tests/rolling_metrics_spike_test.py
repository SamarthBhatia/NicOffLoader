#!/usr/bin/env python3
"""Deterministic spike test validating rolling queue/util metrics."""
...
def verify_metrics(report_path: pathlib.Path) -> None:
    ...
    assert_threshold(queue["average"], 0.5, lambda v, t: v > t, "queue average did not reflect the waiting spike")
    assert_threshold(queue["peak"], 2.0, lambda v, t: v >= t, "queue peak too low")
    assert_threshold(host["average"], 0.3, lambda v, t: v > t, "host utilization average should be elevated")
    assert_threshold(host["peak"], 0.9, lambda v, t: v >= t, "host utilization peak should reach saturation")
    assert_threshold(nic["average"], 0.2, lambda v, t: v < t, "NIC utilization should remain low for host-only spike")
    assert_threshold(sojourn["mean_latency_us"], 60.0, lambda v, t: v > t, "mean sojourn too low for queued tasks")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    default_workload = pathlib.Path("workloads/tests/rolling_spike.yaml")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("[rolling-spike] OK — rolling metrics respond to host saturation")
    return 0
