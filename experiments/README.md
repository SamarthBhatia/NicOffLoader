...
- `policy_baseline/` — batch manifest + docs for `nicloadoff_cli --batch`, running multiple policy configurations, appending metadata-aware rows to a CSV, and providing helpers (`summarize.py`, `export_normalized.py`, `plots/policy_baseline.py`, `import_static_traces.py`) to normalize, visualize, and import the static placement baselines (including normalized CSV/Parquet exports for downstream notebooks).
