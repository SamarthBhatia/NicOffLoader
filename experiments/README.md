...
- `policy_baseline/` — batch manifest + docs for `nicloadoff_cli --batch`, running multiple policy configurations, appending metadata-aware rows to a CSV, and providing helpers (`summarize.py`, `export_normalized.py`, `plots/policy_baseline.py`) to normalize + visualize the results.
