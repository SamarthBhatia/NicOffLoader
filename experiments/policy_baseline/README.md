...
`arrival_label`/`background_load`/`zipf_alpha` **and** the rolling preset/schedule columns (`rolling_preset`, `rolling_schedule_label`) automatically while still supporting metadata-aware filtering/grouping
(`--filter workload_label=skew_dag --group-by policy`) plus extra columns via `--columns`. Its output now includes the
`reorders` column sourced from `policy_metrics.waiting_reorders`, both normalized ratios (`reorders_per_task` across the whole run and `reorders_per_task_recent` over the rolling window), **and** the new admission-related metrics (`admission_blocked`, `admission_limit`, `admission_active`) so you can spot policies that clamp concurrency immediately (grouped views average the counts and ratios). `export_normalized.py`
groups repeated runs, emits both a normalized CSV and (optionally) Parquet table (requires `pyarrow`),
mirrors the full `policy_metrics` struct into those exports, tags each aggregated row with the active `rolling_preset` and `rolling_schedule_label`, derives a `waiting_reorders_per_task`
...
so downstream dashboards can read the normalized schema without extra conversions; it also prints/plots the rolling preset/schedule tags and admission-limit readings so notebook screenshots match the CLI plots.
...
`plots/policy_vs_placement.py` renders the joined table as throughput/latency deltas versus host/NIC placements so you can visualize how policies shift performance relative to static placement baselines. The batch manifest now also includes two DSL-driven runs (`dag-dsl-stage-balance` and `dag-dsl-nic-balance`) referencing the queue-aware configs under `policies/examples/`. They emit `placement=dsl-*-balance` plus a `dsl_policy` metadata tag so the normalized CSV/plots can contrast those DSL heuristics against the built-in prefer-host/prefer-nic policies without extra plot tweaks. The NIC-focused DSL run also pins `rolling_window_preset: nic_util_clamp` so CI exercises the preset metadata/event-log plumbing end-to-end.
...
`plots/policy_baseline.py` consumes the normalized CSV to render throughput/latency comparison charts plus a waiting-reorder subplot that now charts both the cumulative and recent per-task ratios (annotating total and recent-window counts). Additional panels overlay the rolling metrics (queue averages/peaks alongside host/NIC utilization and sojourn quantiles) and the admission-clamp telemetry (blocked-task bars plus the enforced limit) so you can see how each policy shapes short-horizon congestion *and* concurrency caps. All figures land under `plots/generated/`. If you prefer working directly in pandas/BI tooling, see `notebooks/rolling_reorder_example.md` for a lightweight walkthrough that loads `policy_baseline_normalized.csv`, prints the relevant columns, and recreates the side-by-side ratio plot from a notebook; it now also produces queue/utilization and admission summaries whenever the rolling/admission columns are present.
...
That acceptance run now also confirms the `dag-dsl-nic-balance` row reports `rolling_preset=nic_util_clamp`, a `rolling_schedule_label`
prefixed with `preset:`, a non-empty rolling-window event log, and a non-zero `admission_limited_tasks` count (with `admission_limit_last=3`) so preset metadata and the admission-clamp metrics stay under regression coverage right alongside the queue-flip assertions.
