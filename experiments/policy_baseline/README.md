...
mirrors the `policy_metrics.waiting_reorders` counter into those exports, tags each aggregated row with the active `rolling_preset` and `rolling_schedule_label`, derives a `waiting_reorders_per_task`
...
so downstream dashboards can read the normalized schema without extra conversions; it also prints/plots the rolling preset/schedule tags so notebook screenshots match the CLI plots.
...
prefixed with `preset:`, and a non-empty rolling-window event log so preset metadata stays under regression coverage right alongside the queue-flip assertions.
