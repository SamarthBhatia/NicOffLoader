...
- Maintain a running session log in `logs.md` (git-ignored). Each session must append a new entry summarizing the work performed; new sessions pick up from the previous entry so the log remains chronological.
