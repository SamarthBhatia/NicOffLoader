...
- **Next:** Calibrate additional skew variants (80/20 mixes, alternative payload sizes) for the DAG workload and capture their metadata so future policy hooks can reason about dependency pressure explicitly.
- **Remaining:** Compare static host-only vs. NIC-only placements across multiple skew tiers and surface those traces to policy experiments as regression fixtures.
