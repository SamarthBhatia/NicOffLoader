...
- **Next:** Add a regression/analysis check (e.g., in `static_summary_regression_test.py` or a new helper) that ensures the queue-flip prefer-NIC run always reports `waiting_reorders_per_task > 0`, so future policy changes can't silently drop reorder activity.
