...
BasicScheduler::BasicScheduler(ResourcePool resources) : resources_(std::move(resources)) {}
...
void BasicScheduler::handle_task_start(TaskId id, SimTime timestamp) {
    ...
    queue_.push(ScheduledEvent{.timestamp = timestamp + stage.service_time,
...
bool BasicScheduler::try_start_task(TaskContext& ctx, SimTime timestamp) {
    ...
    if (stage.service_time < 0.0) {
        throw SchedulerError(make_error("stage service time cannot be negative", ctx.task.id));
    }
